...
public class MultiplicationService {
	...
	public static String getMultiplicationText(int number1, int number2) {
		...
		stringBuilder.append(
				getSpaces(finalResult.length() - String.valueOf(number1).length()) + number1 + "\n");
		stringBuilder.append(
				getSpaces(finalResult.length() - String.valueOf(number2).length()) + number2 + "\n");
		// need some check
		if(number2 > 9) {
			stringBuilder.append(
		...
		stringBuilder.append(
				getUnderlines(finalResult.length()) + "\n");
	...
	private static String getRows(int number1, int number2) {
		...
		for(int i = symbols.length - 1; i >= 0 ; i--) {
			stringBuilder.append(
					getSpaces(String.valueOf(
							number1*number2).length() - 
